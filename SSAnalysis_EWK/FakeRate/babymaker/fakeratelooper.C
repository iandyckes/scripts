...
//#include "../../CORE/MetSelections.h" //idk if I need this
#include "looper.h" 
...
//Switches
char* path = "../fake_rate_output/V00-00-09";
bool verbose = 0;
...
void babyMaker::MakeBabyNtuple(const char* output_name){
  ...
  BabyTree->Branch("trackerMet", &trackerMet);
  BabyTree->Branch("trackerMetPhi", &trackerMetPhi);
  BabyTree->Branch("sumet", &sumet);
...
pair <float,float> trackerMetFunc(float deltaZCut, std::vector<LorentzVector> jets) {

  if ( tas::vtxs_sumpt().empty() ) return make_pair(-99.9, -99.9);
  float pX = 0;
  float pY = 0;
  
  for (unsigned int i=0; i<tas::pfcands_particleId().size(); ++i)
	{
	  if ( tas::pfcands_charge().at(i)==0 ) continue;
	  if ( jets.size() > 0 )
		{
		  bool matched = false;
		  for ( unsigned int j = 0; j < jets.size(); j++ )
			{
			  if ( fabs(ROOT::Math::VectorUtil::DeltaR(tas::pfcands_p4().at(i), jets.at(j)))<0.5 ) matched=true;
			}
		  if (matched) continue;
		}

	  if ( fabs(tas::pfcands_dz().at(i)) > deltaZCut) continue;
		  
	  pX -= tas::pfcands_p4().at(i).px();
	  pY -= tas::pfcands_p4().at(i).py();
	}
 ...
 //Make Baby Ntuple  
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) { 
    ...
    for(unsigned int evt = 0; evt < nEventsTree; evt++){
	  ...
	  //tackerMet below.  Needs jets
      sumet = cms3.evt_pfsumet();
      metSig = cms3.evt_pfmetSig();
      event = tas::evt_event();
      ...
      //pair <float, float> trackerMetPair = trackerMetFunc(0.2, jets); //using default deltaZ of 0.2//pass empty vector instead for jets.
	  std::vector<LorentzVector> empty;
      pair <float, float> trackerMetPair = trackerMetFunc(0.2, empty); //using default deltaZ of 0.2//pass empty vector instead for jets.
	  trackerMet = trackerMetPair.first;
	  trackerMetPhi = trackerMetPair.second;

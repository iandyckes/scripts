...
protected: 
	...
	float	trackerMet_;
	TBranch *trackerMet_branch;
	bool trackerMet_isLoaded;
	float	trackerMetPhi_;
	TBranch *trackerMetPhi_branch;
	bool trackerMetPhi_isLoaded;
	float	sumet_;
...
void Init(TTree *tree) {
  ...
  tree->SetMakeClass(1);
	...
	trackerMet_branch = 0;
	if (tree->GetBranch("trackerMet") != 0) {
		trackerMet_branch = tree->GetBranch("trackerMet");
		if (trackerMet_branch) {trackerMet_branch->SetAddress(&trackerMet_);}
	}
	trackerMetPhi_branch = 0;
	if (tree->GetBranch("trackerMetPhi") != 0) {
		trackerMetPhi_branch = tree->GetBranch("trackerMetPhi");
		if (trackerMetPhi_branch) {trackerMetPhi_branch->SetAddress(&trackerMetPhi_);}
	}
	sumet_branch = 0;
...
void GetEntry(unsigned int idx) 
	// this only marks branches as not loaded, saving a lot of time
	{
		...
		trackerMet_isLoaded = false;
		trackerMetPhi_isLoaded = false;
	...
	// load all branches
{
	...
	if (trackerMet_branch != 0) trackerMet();
	if (trackerMetPhi_branch != 0) trackerMetPhi();
	...
	float &trackerMet()
	{
		if (not trackerMet_isLoaded) {
			if (trackerMet_branch != 0) {
				trackerMet_branch->GetEntry(index);
			} else { 
				printf("branch trackerMet_branch does not exist!\n");
				exit(1);
			}
			trackerMet_isLoaded = true;
		}
		return trackerMet_;
	}
	float &trackerMetPhi()
	{
		if (not trackerMetPhi_isLoaded) {
			if (trackerMetPhi_branch != 0) {
				trackerMetPhi_branch->GetEntry(index);
			} else { 
				printf("branch trackerMetPhi_branch does not exist!\n");
				exit(1);
			}
			trackerMetPhi_isLoaded = true;
		}
		return trackerMetPhi_;
	}
	float &sumet()
	{
...
namespace samesign {
	const float &met();
	const float &metPhi();
	const float &trackerMet();
	const float &trackerMetPhi();
	const float &sumet();
	const float &metSig();
	const int &event();
	const int &lumi();
